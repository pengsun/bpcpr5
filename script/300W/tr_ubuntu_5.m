...
%% config: model dir
dir_root = pwd;
dir_mo   = fullfile(dir_root, 'mo', 'T12_aug200');
...
rr = [ 0.09*ones(1,6), 0.04*ones(1,6)]; % radius
% for regressor
m = 6; % for hidden layers
K = 68;
knn = [5*ones(1,4), 3*ones(1,4), 1*ones(1,4)]; % for connection mask
% knn = 3*ones(1,T);
